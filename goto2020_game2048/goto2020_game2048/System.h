...
class System {
	...
	double viscousFrictionK = 0.5; // Friction coefficient
	double collisionForce = 5000; // Collison force
	double gravityValue = 10000;
	double collisionK = 0.3; // Collision energy loss
	double initialRadius = 0.065; // Radius of 0-value bubble
	double initialMass = 1; // Mass of 0-value bubble
	
	int score = 0;
	int typeNumber = 2; // Number of types
	int cellNumber = 1; // Number of cells vertically and horizontally
	double cellSize = 600; // Side of single cells
...
public:
	...
	std::vector<Animation> get_animations(); // Returns copy of animations array
	double get_cellSize();
